...
public interface FriendshipRepository extends JpaRepository<Friendship, String> {
    ...
    @Query("SELECT f FROM Friendship f WHERE (f.sender = :user1 AND f.receiver = :user2) OR (f.sender = :user2 AND f.receiver = :user1) ORDER BY f.requestedAt DESC")
    ...
    Optional<Friendship> findByUsersAndStatus(@Param("user1") User user1, @Param("user2") User user2,
            @Param("status") FriendshipStatus status);

    Page<Friendship> findBySenderAndStatusOrderByRequestedAtDesc(User sender, FriendshipStatus status,
            Pageable pageable);

    Page<Friendship> findByReceiverAndStatusOrderByRequestedAtDesc(User receiver, FriendshipStatus status,
            Pageable pageable);
